...
spawn_claude_pr_review() {
    ...
    (
        ...
        if [[ -x "${HOME}/.asdf/installs/nodejs/22.14.0/bin/claude" ]]; then
            ...
            echo "Please install Claude Code: https://claude.ai/download" >> "$log_file"
            # Send notification about the error
            osascript -e 'display notification "Claude CLI not found. Please install Claude Code." with title "PR Review Error" sound name "Basso"' 2>/dev/null || true
            exit 1
        fi
        ...
        "$claude_cmd" --print --dangerously-skip-permissions "$prompt" >> "$log_file" 2>&1
